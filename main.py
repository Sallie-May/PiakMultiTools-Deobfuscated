...
def PIAK_tool2():
    ...
    ctypes.windll.kernel32.SetConsoleTitleW(f'{toolname} - Made By {owners} - Date : {date_now} - Logged As : {username} | Tokens : {tokens} | https://github.com/PiakCorp/PiakMultiTools')
    Write.Print(f"""
                                       ██▓███   ██▓ ▄▄▄       ██ ▄█▀
                                      ▓██░  ██▒▓██▒▒████▄     ██▄█▒ 
                                      ▓██░ ██▓▒▒██▒▒██  ▀█▄  ▓███▄░ 
                                      ...
                                      ░░        ▒ ░  ░   ▒   ░ ░░ ░ 
                                                ░        ░  ░░  ░   
                                Welcome {username} | https://discord.gg/wVUMsHAHeJ
                            ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════
        ...
        \t(34) Bot Server Nuker""", Colors.yellow_to_red, interval=0.0000); print(pretty + f"", end=''); Write.Print(f"\n                (<<) Last Page\n", Colors.purple_to_blue, interval=0.000)
    Write.Print(f"""════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════""", Colors.yellow_to_red, interval=0.0000)
    Write.Print(f"\nroot@PIAKmultitool ~> ", Colors.yellow_to_red, interval=0.000); opc = input(magenta).lower()
    if opc=="<" or opc=="<<":
        PIAK_tool()
    ...
    if opc=="34":
        ...
        def server_nuker():
            ...
            async def on_ready():
                System.Clear()
                a = discord.Game(name=f"PiakTool est ici", type=3)
                ...
                async def PIAK_start(ctx):
                    ...
                    for role in ctx.guild.roles:
                        try:
                            await role.delete()
                            time_rn = get_time_rn()
                            print(f"{reset}[ {cyan}{time_rn}{reset} ] {gray}({green}+{gray}) {pretty}Rôle Supprimé {gray}---> {pink}{role.name}")
                        except:
                            pass
                            time_rn = get_time_rn()
                            print(f"{reset}[ {cyan}{time_rn}{reset} ] {gray}({red}-{gray}) {pretty}Échec de la suppression du rôle {gray}---> {pink}{role.name}")
                    
                    for channel in ctx.guild.channels:
                        try:
                            await channel.delete()
                            time_rn = get_time_rn()
                            print(f"{reset}[ {cyan}{time_rn}{reset} ] {gray}({green}+{gray}) {pretty}Salon Supprimée {gray}---> {pink}#{channel.name}")
                        except:
                            pass
                            time_rn = get_time_rn()
                            print(f"{reset}[ {cyan}{time_rn}{reset} ] {gray}({red}-{gray}) {pretty}Échec de la suppression du salon {gray}---> {pink}#{channel.name}")
                    
                    try:
                        for i in range(69):
                            await ctx.guild.create_text_channel(channel_names)
                            time_rn = get_time_rn()
                            print(f"{reset}[ {cyan}{time_rn}{reset} ] {gray}({green}+{gray}) {pretty}Salon Créé {gray}---> {pink}#{channel_names}")
                    except Exception:
                        time_rn = get_time_rn()
                        print(f"{reset}[ {cyan}{time_rn}{reset} ] {gray}({red}-{gray}) {pretty}Échec de la création du salon {gray}---> {pink}#{channel_names}")
            try:
                PIAKtool_bot.run(bot_token)
...
def PIAK_tool():
    ...
    System.Clear()
    tokens = len(open('tokens.txt').readlines())
    ctypes.windll.kernel32.SetConsoleTitleW(f'{toolname} - Made By {owners} - Date : {date_now} - Logged As : {username} | Tokens : {tokens} |')
    Write.Print(f"""
                                       ██▓███   ██▓ ▄▄▄       ██ ▄█▀
                                      ▓██░  ██▒▓██▒▒████▄     ██▄█▒ 
                                      ▓██░ ██▓▒▒██▒▒██  ▀█▄  ▓███▄░ 
...
════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════
        ...
        \t(10) Friend Spammer\t\t(20) Server Leaver\t\t""", Colors.yellow_to_red, interval=0.0000); print(pretty + f"", end=''); Write.Print(f"(>>) Next Page\n", Colors.purple_to_blue, interval=0.000)
    Write.Print(f"""════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════""", Colors.yellow_to_red, interval=0.0000)
    Write.Print(f"\nroot@PIAKmultitool ~> ", Colors.yellow_to_red, interval=0.000); opc = input(magenta).lower()
    ...
    if opc=="1":
        ...
        Write.Print(f"""\n\n
                                    ╔═════════════════════════════════════════╗
                                    ║   PIAK MULTI-TØØL TOKEN CHECKER STATS  ║
                                ╔═════════════════════════════════════════════════╗
...
""", Colors.purple_to_red, interval=0.000)
    ...
    if opc=="15":
        ...
        def channel_spammer(token, msg, channel_id):
            if emojis == True:
                selected_emojis = random.sample(PIAKlist, 12)
    ...
    if opc=="19":
        ...
        try:
            ...
            pass
        Write.Print(f"\nroot@token ~> ", Colors.yellow_to_red, interval=0.000); token_s = input(magenta)
...
════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════
""", Colors.purple_to_red, interval=0.000)
        Write.Print(f"\nroot@PIAKmultitool ~> ", Colors.yellow_to_red, interval=0.000); opc2 = input(magenta).lower()
        if opc2=="1":
        ...
        if opc2=="4":
            ...
            def role_deleter(role_id, headers, role_name, server_id, output_lock):
                ...
                if r2.status_code != 204:
                    ...
                    with output_lock:
                        time_rn = get_time_rn()
                        print(f"{reset}[ {cyan}{time_rn}{reset} ] {gray}({green}+{gray}) {pretty}Rôle Supprimé {gray}| ", end='')
                        sys.stdout.flush()
